...
def deep_dive_variable_interactions(df):
    fig, axes = plt.subplots(4, 2, figsize=(20, 28))
    plt.subplots_adjust(hspace=0.4, wspace=0.3)
    fig.suptitle("Deep Dive: Variable Interactions", fontsize=20)

    # 1. Categorical × Categorical → Target
    ct1 = pd.crosstab(df['segment'], df['transmission_type'], values=df['is_claim'], aggfunc='mean')
    sns.heatmap(ct1, annot=True, fmt=".2f", cmap='coolwarm', ax=axes[0, 0])
    axes[0, 0].set_title("Claim Rate: Segment vs Transmission Type")

    # 2. Categorical × Categorical → Count
    sns.countplot(data=df, x='fuel_type', hue='transmission_type', ax=axes[0, 1])
    axes[0, 1].set_title("Fuel Type vs Transmission Type")
    axes[0, 1].tick_params(axis='x', rotation=45)

    # 3. Categorical × Numerical → Target
    sns.boxplot(data=df, x='segment', y='power_value', hue='is_claim', ax=axes[1, 0])
    axes[1, 0].set_title("Power Value by Segment and Claim Status")
    axes[1, 0].tick_params(axis='x', rotation=45)

    # 4. Numerical × Numerical → Target
    sns.scatterplot(data=df, x='power_value', y='torque_value', hue='is_claim', alpha=0.6, ax=axes[1, 1])
    axes[1, 1].set_title("Power vs Torque Colored by Claim")

    # 5. Categorical × Numerical (Mean) → Target
    top_models = df['model'].value_counts().head(10).index
    df_top = df[df['model'].isin(top_models)]
    sns.pointplot(data=df_top, x='model', y='power_value', hue='is_claim', dodge=True, ax=axes[2, 0])
    axes[2, 0].set_title("Mean Power by Model and Claim Status")
    axes[2, 0].tick_params(axis='x', rotation=45)

    # 6. Categorical × Ordinal → Target (if available)
    if 'ncap_rating' in df.columns:
        ct2 = pd.crosstab(df['segment'], df['ncap_rating'], values=df['is_claim'], aggfunc='mean')
        sns.heatmap(ct2, annot=True, fmt=".2f", cmap='YlOrRd', ax=axes[2, 1])
        axes[2, 1].set_title("Claim Rate: Segment vs NCAP Rating")

    # 7. Distribution of Torque by Fuel Type and Claim
    sns.boxplot(data=df, x='fuel_type', y='torque_value', hue='is_claim', ax=axes[3, 0])
    axes[3, 0].set_title("Torque by Fuel Type and Claim Status")
    axes[3, 0].tick_params(axis='x', rotation=45)

    # 8. Segment vs Engine Type → Target
    ct3 = pd.crosstab(df['segment'], df['engine_type'], values=df['is_claim'], aggfunc='mean')
    sns.heatmap(ct3, annot=True, fmt=".2f", cmap='RdYlGn_r', ax=axes[3, 1])
    axes[3, 1].set_title("Claim Rate: Segment vs Engine Type")
...
def run_pca(df, target_col='is_claim', n_components=2):
    # Step 1: Select numeric features (excluding the target)
    features = df.select_dtypes(include=['int', 'float']).drop(columns=[target_col])
    ...
    # Step 3: Apply PCA
    pca = PCA(n_components=None)  # get all components to analyze variance
    principal_components = pca.fit_transform(X_scaled)
    
    # Step 4: Create a DataFrame for the components
    all_pcs_df = pd.DataFrame(data=principal_components, columns=[f'PC{i+1}' for i in range(principal_components.shape[1])])
    all_pcs_df[target_col] = df[target_col].values

    # Step 5: Scatterplot using the first 2 PCs
    plt.figure(figsize=(10, 6))
    sns.scatterplot(x='PC1', y='PC2', hue=target_col, data=all_pcs_df, palette='Set1', alpha=0.6)
    plt.title('PCA: First 2 Principal Components Colored by Claim Status')
    plt.xlabel('PC1')
    plt.ylabel('PC2')
    ...
    # Step 6: Plot explained variance for all components
    ...
    # Step 7: Print cumulative variance
    cumulative_variance = explained_variance.cumsum()
    for i, var in enumerate(cumulative_variance):
        print(f"Cumulative variance by PC1 to PC{i+1}: {var:.4f}")
    
    # Return the PCA object and scaled data (optional)
    return pca, X_scaled, principal_components

def find_best_k_elbow_silhouette(df, target_col='is_claim', max_k=10):
    features = df.select_dtypes(include=['int', 'float']).drop(columns=[target_col])
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(features)
    ...
    print(f"Best k by silhouette score: {K[np.argmax(sil_scores)]}")

def run_kmeans(df, target_col='is_claim', n_clusters=3):
    features = df.select_dtypes(include=['int', 'float']).drop(columns=[target_col])
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(features)
    ...
    plt.figure(figsize=(10, 6))
    sns.scatterplot(data=plot_df, x='PC1', y='PC2', hue='Cluster', palette='viridis', alpha=0.7)
    plt.title(f'K-Means Clusters (k={n_clusters}) on First 2 PCA Components')
    plt.xlabel('PC1')
    plt.ylabel('PC2')
    ...
    print("Cluster vs Target Crosstab:")
    print(pd.crosstab(plot_df['Cluster'], plot_df[target_col]))

    score = silhouette_score(X_scaled, cluster_labels)
    print(f'Silhouette Score for k={n_clusters}: {score:.3f}')
    
    return kmeans, pca, cluster_labels, plot_df

def run_dbscan(df, target_col='is_claim', eps=0.5, min_samples=5):
    features = df.select_dtypes(include=['int', 'float']).drop(columns=[target_col])
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(features)
    ...
    plt.figure(figsize=(10, 6))
    sns.scatterplot(data=plot_df, x='PC1', y='PC2', hue='Cluster', palette='Spectral', alpha=0.7)
    plt.title(f'DBSCAN Clusters (eps={eps}, min_samples={min_samples}) on PCA Components')
    plt.xlabel('PC1')
    plt.ylabel('PC2')
    plt.legend(title='Cluster')
    plt.grid(True)
    ...
    print("Cluster vs Target Crosstab (noise = -1):")
    print(pd.crosstab(plot_df['Cluster'], plot_df[target_col]))

    return dbscan, pca, cluster_labels, plot_df

def run_hierarchical(original_df=None, target_col=None, n_clusters=0):
    features = original_df.select_dtypes(include=['int', 'float']).drop(columns=[target_col])
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(features)
    ...
    if original_df is not None and target_col is not None:
        plot_df[target_col] = original_df[target_col].values

    # Plot
    plt.figure(figsize=(10, 6))
    sns.scatterplot(data=plot_df, x='PC1', y='PC2', hue='Cluster', palette='Set2', alpha=0.7)
    plt.title('Hierarchical Clustering (Agglomerative) - PCA 2D View')
