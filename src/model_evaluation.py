...
def plot_evaluation_summary(y_true, y_pred):
    ...
    # Plot 1: Actual vs Predicted side-by-side bar chart with labels
    ...
    bar_plot = bar_df.plot(kind='bar', ax=axs[0], width=0.7)
    axs[0].set_title("Actual vs Predicted Claims")
    axs[0].set_ylabel("Count")
    axs[0].set_xlabel("Class (0 = No Claim, 1 = Claim)")
    axs[0].legend(title="Legend")
    axs[0].tick_params(axis='x', rotation=0)

    # Add value labels on top of bars
    for container in bar_plot.containers:
        bar_plot.bar_label(container, label_type='edge', padding=3, fontsize=10)
...
def tune_threshold_by_class_distribution(y_true, y_proba, target_class_0_ratio=(0.92, 0.94), step=0.01):
    best_thresh = 0.5
    best_f1 = 0
    ...
    for thresh in np.arange(0.1, 0.9, step):
        ...
        pred_class_0_ratio = (y_pred_thresh == 0).mean()

        # How close is predicted class 0 ratio to target range?
        dist_diff = 0
        if pred_class_0_ratio < target_class_0_ratio[0]:
            dist_diff = target_class_0_ratio[0] - pred_class_0_ratio
        elif pred_class_0_ratio > target_class_0_ratio[1]:
            dist_diff = pred_class_0_ratio - target_class_0_ratio[1]

        f1 = f1_score(y_true, y_pred_thresh, average='macro')

        # Prioritize thresholds within target class 0 ratio, then by best F1
        if dist_diff == 0 and f1 > best_f1:
            best_f1 = f1
            best_thresh = thresh
            best_dist_diff = dist_diff
        elif dist_diff < best_dist_diff:  # get closest even if no exact match
            best_f1 = f1
            best_thresh = thresh
            best_dist_diff = dist_diff

    print(f"Tuned threshold: {best_thresh:.2f}")
    print(f"F1 macro at tuned threshold: {best_f1:.4f}")
    return best_thresh
